...
export function write_uint(
  ...
  switch (type) {
    case Type.UINT8:
      buf.writeUInt8(value as number, pos);
      return;
    case Type.UINT16:
      buf.writeUInt16BE(value as number, pos);
      return;
    case Type.UINT32:
      buf.writeUInt32BE(value as number, pos);
      return;
    case Type.UINT64:
      buf.writeBigUInt64BE(value as bigint, pos);
      return;
  }
}

export function write_byte(buf: Buffer, value: Buffer, pos: number): void {
  value.copy(buf, pos);
}

export function write_utf8(buf: Buffer, value: string, pos: number): void {
  buf.write(value, pos, "utf8");
}

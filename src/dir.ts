...
export type BitcaskKeyDir = Map<string, Buffer>;
export const BitcaskKeyDir: typeof Map<string, Buffer> = Map;

export function dir_write(
  file_id: number,
  record_sz: number,
  record_pos: number,
  epoch: bigint,
...
export function dir_read(buf: Buffer): BitcaskDir {
  let pos = 0;
